...
import { fetchPlayerTournamentHistory, setFilters } from '../../slices/tournamentHistorySlice'
import PlayerSidebar from './PlayerSidebar'
import ProfileMenu from '../../components/common/ProfileMenu'
import PdfExportButton from '../../components/common/PdfExportButton'
import { Calendar, Coins, Filter, Search, Trophy } from 'lucide-react'
...
const PlayerTournamentHistory = () => {
  ...
  const { playerHistory, filters } = useAppSelector((state) => state.tournamentHistory)
  const [localFilters, setLocalFilters] = useState(filters)
  const [search, setSearch] = useState('')

  const { data: tournaments, count, completed, ongoing, registered, loading, error } = playerHistory

  useEffect(() => {
    dispatch(fetchPlayerTournamentHistory(filters))
  }, [dispatch, filters])
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-[#0F172A] border-b border-[#1F2937] px-8 py-5 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-[#9CA3AF] mt-1">Track all tournaments you joined</p>
          </div>
          <ProfileMenu />
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-4">
              <p className="text-xs text-[#9CA3AF]">Total Tournaments</p>
              <p className="text-2xl font-bold text-[#E5E7EB] mt-1">{count || 0}</p>
            </div>
            <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-4">
              <p className="text-xs text-[#9CA3AF]">Completed</p>
              <p className="text-2xl font-bold text-emerald-600 mt-1">{completed || 0}</p>
            </div>
            <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-4">
              <p className="text-xs text-[#9CA3AF]">Ongoing</p>
              <p className="text-2xl font-bold text-amber-600 mt-1">{ongoing || 0}</p>
            </div>
            <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-4">
              <p className="text-xs text-[#9CA3AF]">Registration Open</p>
              <p className="text-2xl font-bold text-blue-600 mt-1">{registered || 0}</p>
            </div>
          </div>
          ...
          <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-4">
            <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
              ...
              <select
                ...
                className="px-3 py-2.5 bg-[#0F172A] border border-[#1F2937] rounded-lg text-sm text-[#E5E7EB]"
              >
              ...
              <select
                ...
                onChange={(e) => setLocalFilters((prev) => ({ ...prev, gameType: e.target.value || null }))}
                className="px-3 py-2.5 bg-[#0F172A] border border-[#1F2937] rounded-lg text-sm text-[#E5E7EB]"
              >
              ...
              <select
                ...
                onChange={(e) => setLocalFilters((prev) => ({ ...prev, month: e.target.value ? parseInt(e.target.value) : null }))}
                className="px-3 py-2.5 bg-[#0F172A] border border-[#1F2937] rounded-lg text-sm text-[#E5E7EB]"
              >
                {MONTHS.map((m) => (
              ...
              <select
                ...
                onChange={(e) => setLocalFilters((prev) => ({ ...prev, year: parseInt(e.target.value) }))}
                className="px-3 py-2.5 bg-[#0F172A] border border-[#1F2937] rounded-lg text-sm text-[#E5E7EB]"
              >
            ...
            <div className="flex items-center gap-2 mt-4">
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm rounded-lg"
              >
              ...
              <button
                ...
                className="px-4 py-2 border border-[#374151] text-[#E5E7EB] text-sm rounded-lg hover:bg-[#1F2937]"
              >
                Reset
              </button>
              <PdfExportButton
                reportTitle="Player Tournament History"
                stats={pdfStats}
                columns={pdfColumns}
                rows={pdfRows}
                ...
                <table className="min-w-full text-sm">
                  <thead className="bg-[#0F172A] text-[#9CA3AF]">
                    <tr>
                      ...
                      <th className="text-left px-4 py-3 font-medium">Entry Fee</th>
                      <th className="text-left px-4 py-3 font-medium">Prize Pool</th>
